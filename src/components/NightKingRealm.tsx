...
export const NightKingRealm: React.FC<NightKingRealmProps> = ({ isSoundMuted, onMelt }) => {
  ...
  // Flying Dragon and fullscreen fire melting transition
  const handleMeltTransition = () => {
    if (isMelting) return;
    setIsMelting(true);

    // Synthesize dramatic, roaring flame breath sound
    if (!isSoundMuted) {
      try {
        const AudioContext = window.AudioContext || (window as any).webkitAudioContext;
        const ctx = new AudioContext();

        // 1. Dragon Roar Low Pitch Rumble
        ...
        osc1.frequency.exponentialRampToValueAtTime(75, ctx.currentTime + 1.2);

        osc2.type = 'triangle';
        osc2.frequency.setValueAtTime(90, ctx.currentTime);
        osc2.frequency.linearRampToValueAtTime(55, ctx.currentTime + 1.2);

        rumbleGain.gain.setValueAtTime(0.35, ctx.currentTime);
        rumbleGain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 1.6);

        osc1.connect(rumbleGain);
        osc2.connect(rumbleGain);
        rumbleGain.connect(ctx.destination);

        // 2. Fiery Rush / Sizzling White Noise for Fire Spray
        ...
        noiseFilter.frequency.exponentialRampToValueAtTime(800, ctx.currentTime + 1.5);
        ...
        console.warn("Roar audio failed to play:", err);
      }
    }

    // Return to Summer Portfolio after the animation is completed
    setTimeout(() => {
      onMelt();
    }, 2400);
  };
  ...
  return (
    <div id="night-king-realm-container" className="fixed inset-0 z-50 overflow-y-auto select-none font-sans bg-slate-950 text-slate-100">
      ...
      {/* Full-screen melting fire animation */}
      <AnimatePresence>
        {isMelting && (
          <div className="fixed inset-0 pointer-events-none z-[100] overflow-hidden">
            
            {/* Swooping Majestic 3D Dragon */}
            <motion.div
              initial={{ x: '-60vw', y: '25vh', rotateY: 45, rotateX: 15, rotateZ: 8, scale: 1.3 }}
              animate={{
                x: ['-60vw', '12vw', '15vw', '120vw'],
                y: ['25vh', '28vh', '22vh', '10vh'],
                rotateY: [45, 15, -10, -45],
                rotateX: [15, 8, 12, 5],
                rotateZ: [8, 2, -2, -10],
              }}
              ...
              className="absolute w-[450px] h-[450px] pointer-events-none z-[120] flex items-center justify-center drop-shadow-[0_0_60px_rgba(239,68,68,0.85)]"
            >
              <svg viewBox="0 0 200 200" fill="currentColor" className="w-full h-full transform -scale-x-100">
                <defs>
                  ...
                  <linearGradient id="dragonFireGrad" x1="0%" y1="0%" x2="100%" y2="50%">
                ...
                >
                  <path
                    d="M 110 90 
                       C 135 45, 165 35, 195 45 
                       C 185 75, 155 95, 110 90 Z"
                    fill="url(#dragonWingGrad)"
                    opacity="0.85"
                  />
                  <path d="M 110 90 L 195 45" stroke="#020617" strokeWidth="2.5" />
                  <path d="M 110 90 L 175 70" stroke="#020617" strokeWidth="1.5" />
                </motion.g>

                {/* Tail */}
                <path
                  d="M 110 105 
                     C 130 115, 150 125, 165 115
                     C 180 105, 185 85, 195 75"
                  fill="none"
                  stroke="url(#dragonScaleGrad)"
                  strokeWidth="10"
                  strokeLinecap="round"
                />
                {/* Spines on tail */}
                <path d="M 130 111 L 133 105 L 136 111 Z M 150 120 L 153 113 L 156 120 Z M 170 110 L 173 103 L 176 110 Z" fill="#fbbf24" />

                {/* Main Body */}
                <path
                  d="M 110 105
                     C 90 100, 85 85, 95 75
                     C 105 65, 120 85, 110 105 Z"
                ...
                <path
                  d="M 98 78
                     C 92 84, 90 92, 98 100
                     C 104 100, 108 90, 98 78 Z"
                ...
                {/* Head (with animated roaring jaw) */}
                <g transform="translate(60, 55)">
                  {/* Horns */}
                  <path d="M 0 -5 C 15 -20, 30 -25, 45 -20" stroke="#0f172a" strokeWidth="4" fill="none" />
                  <path d="M 0 0 C 10 -12, 22 -16, 35 -12" stroke="#0f172a" strokeWidth="3" fill="none" />
                  
                  {/* Head Skull */}
                  <path d="M -10 -10 L 10 -15 L 15 0 L -10 10 Z" fill="url(#dragonScaleGrad)" />
                  <circle cx="2" cy="-5" r="2.5" fill="#facc15" className="animate-pulse" />

                  {/* Upper Snout */}
                  <path d="M -10 -10 C -20 -10, -35 -5, -38 2 C -35 8, -15 8, -10 2 Z" fill="url(#dragonScaleGrad)" />

                  {/* Roaring Lower Jaw */}
                  <motion.path
                    d="M -10 2 L -35 2 L -25 15 Z"
                    fill="url(#dragonBellyGrad)"
                    stroke="#020617"
                    strokeWidth="0.8"
                    animate={{
                ...
                >
                  <path
                    d="M 95 95 
                       C 60 50, 30 40, 5 50 
                       C 20 85, 55 105, 95 95 Z"
                    fill="url(#dragonWingGrad)"
                  />
                  <path d="M 95 95 L 5 50" stroke="#020617" strokeWidth="3" />
                  <path d="M 95 95 L 35 80" stroke="#020617" strokeWidth="2" />
                </motion.g>

                {/* Massive Animated Fire Stream */}
                <motion.g
                ...
                >
                  {/* Huge expanding fire cone path */}
                  <path
                    d="M 22 55 
                       C -60 20, -120 10, -180 5 
                       C -195 25, -195 85, -180 105 
                       C -120 100, -60 90, 22 55 Z"
                    fill="url(#dragonFireGrad)"
                    filter="url(#fireGlow)"
                  />
                  {/* Inner extra hot white core */}
                  <path
                    d="M 22 55 
                       C -40 35, -80 30, -130 25 
                       C -140 40, -140 70, -130 85 
                       C -80 80, -40 75, 22 55 Z"
                    fill="#ffffff"
                    opacity="0.8"
                    filter="url(#fireGlow)"
                  />
            ...
            {/* Giant fullscreen blending fire wave sweep */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{
                opacity: [0, 0.95, 1.0, 1.0, 0],
              }}
              transition={{
                duration: 2.4,
                times: [0, 0.35, 0.5, 0.8, 1.0],
                ease: "easeInOut"
              }}
              className="absolute inset-0 bg-gradient-to-tr from-red-700 via-orange-600 to-yellow-400 mix-blend-screen"
            >
              {/* Sizzling spark embers floating upward across the viewport */}
              <div className="absolute inset-0 flex flex-wrap justify-around items-center overflow-hidden">
                {Array.from({ length: 45 }).map((_, i) => (
                  <motion.div
                    ...
                    className="absolute w-5 h-5 rounded-full bg-yellow-400 shadow-[0_0_20px_rgba(251,146,60,0.9),0_0_40px_rgba(239,68,68,0.7)]"
                    style={{
            ...
            {/* Cinematic flash to pure absolute energy at peak melt moment */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{
              ...
              transition={{
                duration: 2.4,
                times: [0, 0.45, 0.55, 0.75, 1.0],
                ease: "easeInOut"
              }}
              className="absolute inset-0 bg-gradient-to-b from-amber-100 via-white to-orange-100 mix-blend-color-dodge z-[110]"
            />
          </div>
        )}
      </AnimatePresence>
